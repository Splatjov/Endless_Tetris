...
//                               0x44C0
//-------------------------------------------------------------------------
const i = { size: 4, blocks: [0x0F00, 0x8888, 0x00F0, 0x8888], color: 'cyan'   };
const j = { size: 3, blocks: [0x44C0, 0x8E00, 0xC880, 0x0E20], color: 'blue'   };
const l = { size: 3, blocks: [0x88C0, 0x0E80, 0xC440, 0x2E00], color: 'orange' };
const o = { size: 2, blocks: [0xCC00, 0xCC00, 0xCC00, 0xCC00], color: 'yellow' };
const s = { size: 3, blocks: [0x06C0, 0x8C40, 0x6C00, 0x8C40], color: 'green'  };
const t = { size: 3, blocks: [0x0E40, 0x4C40, 0x4E00, 0x8C80], color: 'purple' };
const z = { size: 3, blocks: [0x0C60, 0x4C80, 0xC600, 0x4C80], color: 'red'    };
...
function unoccupied(type, x, y, dir) {
    return !occupied(type, x, y, dir, blocks);
}
...
function drawScore() {
    if (invalid.score) {
        html('score', ("0000000" + Math.floor(vscore)).slice(-7));
        invalid.score = false;
    }
}
function drawMean() {
    ...
    mean += 1;
    summean += score;
    html('mean', ("0000000" + Math.floor(summean/mean)).slice(-7));
}
...
function agent() {
    ...
    if (bestMove) {
        let dropY = getDropPosition(bestMove.piece, bestMove.x, blocks);
